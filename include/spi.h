 ...
 *   half_duplex: nonzero in case the slave is asynchronous and replies to
 *		commands with (possibly long) delays
 *   frame_header: the first byte of the frames generated by the slave in case
 *		it is free running
 *   max_timeout_ms: maximum timeout to wait for reply in case of free
 *		running slave, in milliseconds
 */
struct spi_slave {
	unsigned int	bus;
	unsigned int	cs;
