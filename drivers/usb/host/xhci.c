 ...
 * @return 0 on success, failure not implemented
 */
static int xhci_configure_endpoints(struct usb_device *udev, bool ctx_change)
{
	...
	xhci_queue_command(ctrl, in_ctx->bytes, udev->slot_id,
...
static int xhci_address_device(struct usb_device *udev)
{
	...
	xhci_queue_command(ctrl, (void *)ctrl_ctx, slot_id, TRB_ADDR_DEV);
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
...
int usb_alloc_device(struct usb_device *udev)
{
	...
	xhci_queue_command(ctrl, NULL, 0, TRB_ENABLE_SLOT);
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
